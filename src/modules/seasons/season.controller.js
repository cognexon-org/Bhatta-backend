...
exports.create = asyncHandler(async (req, res) => { const payload = { ...req.body, createdBy: req.user._id }; if (req.user.role === roles.MANAGER && req.user.assignedKilnId) payload.kilnId = req.user.assignedKilnId; if (payload.isDefault || payload.status === 'ACTIVE') await Season.updateMany({ kilnId: payload.kilnId }, { $set: { isDefault: false, status: 'CLOSED' } }); const item = await Season.create(payload); return created(res, t('CREATED', req.lang), item); });
