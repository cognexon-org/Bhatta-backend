...
async function createNotification({ userId, title, message, type = 'GENERAL', relatedModule, relatedId, sendPush = true, sendSmsToMobile = false }) {
  ...
  if (sendPush && user?.fcmToken) {
    try { await sendPushNotification({ token: user.fcmToken, title, body: message, data: { notificationId: notification._id, relatedModule, relatedId } }); } catch (error) { console.warn('Push notification failed:', error.message); }
  }
