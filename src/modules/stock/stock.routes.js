...
router.get('/summary', permit(roles.ADMIN, roles.MANAGER), controller.stockSummary);
router.get('/', permit(roles.ADMIN, roles.MANAGER), controller.listStock);
router.post('/requests', permit(roles.ADMIN, roles.MANAGER), [body().custom((value) => { if (!value.categoryId && !value.categoryCode) throw new Error('categoryId or categoryCode is required'); return true; }), body('updateType').notEmpty(), body('quantity').isNumeric()], validate, controller.createStockRequest);
