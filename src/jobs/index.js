...
const { runJobForAllTenants } = require('../platform/tenantJobRunner');

function runSafely(name, fn) {
  runJobForAllTenants(name, fn)
    .then((r) => console.log(`[job:${name}]`, r))
    .catch((e) => console.error(`[job:${name}] failed`, e.message));
}
function startJobs() {
  if (process.env.NODE_ENV === 'test' || String(process.env.DISABLE_CRON || 'false').toLowerCase() === 'true') return;
  ...
  console.log('Tenant-aware cron jobs scheduled');
}
